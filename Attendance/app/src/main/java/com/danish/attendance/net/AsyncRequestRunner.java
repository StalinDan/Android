...
import com.danish.attendance.BuildConfig;
import com.danish.attendance.activity.MainActivity;
import com.danish.attendance.utils.DeviceUtil;
...
//        }.start();
    ...
    private Request.Builder packageBuild(Request.Builder builder) {
        ...
        builder.addHeader("bid", SharedPreferenceManager.getBindNumber());
        return builder;
    }
     ...
     * @param params     参数
     * @param tag        tag,主要用来取消
     * @param downLoadCallBack   回调监听
     */
    public void request(final int requestWay, final String url, final ArrayMap<String, String> params, final String tag, final MainActivity.DownLoadCallBack downLoadCallBack) {
        ...
        Observer observer = new Observer<byte[]>() {
            ...
            public void onNext(byte[] bytes) {
                if (downLoadCallBack != null) {
                    downLoadCallBack.downLoadSuccess(bytes);
                }
            }
        ...
        Observable.create(new ObservableOnSubscribe<byte[]>() {
            ...
            public void subscribe(ObservableEmitter<byte[]> e) throws Exception {
                ...
                if (response != null && response.isSuccessful()) {
                    MyLogger.i("   " + response.code());
                    try {
                        e.onNext(response.body().bytes());
                        Log.i("aa","byte------->" + response.body().bytes().length);
                    } catch (IOException e1) {
                        e.onError(e1);
                    }
                } else {
